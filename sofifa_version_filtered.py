...
class SoFIFA(BaseSeleniumReader):
    ...
    versions : string, int or list of int, optional
        FIFA/FC release to include. Human-readable names such as ``"FIFA 16"``,
        ``"fifa16"``, ``"FC 24"``, and ``"fc24"`` are accepted and normalized.
        For a named edition, only its newest SoFIFA rating update is selected.
        Raw SoFIFA version IDs (e.g., 230034), lists of IDs, ``"all"``, and
        ``"latest"`` retain their original behavior. Defaults to ``"latest"``.
    ...
    def __init__(
        ...
        available_versions = self.read_versions()

        if versions == "latest":
            self.versions = available_versions.tail(n=1)

        elif versions == "all":
            self.versions = available_versions

        elif isinstance(versions, int):
            # Preserve the original behavior for raw SoFIFA version IDs.
            self.versions = available_versions.loc[[versions]]

        elif isinstance(versions, list) and all(isinstance(v, int) for v in versions):
            # Preserve the original behavior for a list of raw SoFIFA version IDs.
            self.versions = available_versions.loc[versions]

        elif isinstance(versions, str):
            # Accept human-readable game names such as:
            # fifa16, FIFA16, FIFA 16, fifa 16 -> FIFA 16
            # fc24, FC24, fC24, FC 24       -> FC 24
            requested_edition = self._normalize_edition_name(versions)

            matching_versions = available_versions[
                available_versions["fifa_edition"]
                .astype(str)
                .map(self._normalize_edition_name)
                .eq(requested_edition)
            ]

            if matching_versions.empty:
                available_editions = sorted(
                    available_versions["fifa_edition"].astype(str).unique()
                )
                raise ValueError(
                    f"Could not find SoFIFA edition {requested_edition!r}. "
                    f"Available editions include: {available_editions}"
                )

            # Each FIFA/FC edition can contain many SoFIFA rating updates.
            # Use only the newest update belonging to the requested edition.
            self.versions = matching_versions.tail(n=1)
    ...
    def _normalize_edition_name(edition: str) -> str:
        """Normalize a FIFA/FC edition name to SoFIFA's display format.
        ...
        if not isinstance(edition, str):
            raise TypeError("Edition name must be a string.")

        compact = re.sub(r"[^a-zA-Z0-9]", "", edition).upper()

        match = re.fullmatch(r"(FIFA|FC)(\\d{2})", compact)
        if match is None:
            raise ValueError(
                f"Invalid FIFA/FC edition name: {edition!r}. "
                "Use a value such as 'FIFA 16', 'fifa16', 'FC 24', or 'fc24'."
            )

        game, year = match.groups()
        return f"{game} {year}"
